...
if(length(args)==0){
  ...
  sce     <- "nbcpcoupling"
  sim     <- "simu1"
  igraph  <- "a_graph100" #check graph number in C:\Users\mruf\Documents\PHD_projects\Proj_3\DISPLACE_input_BalticSea\simusspe_BalticSea\scenbcpcoupling
}else{
  pop      <- args[1]
  tstep    <- args[2]
  sce      <- args[3]
...
FRANCOIS <- FALSE ; MARIECHRISTINE <- TRUE
...
if(TRUE){
  ...
  NSIM <- 500 #Choose the number of simulations and take the mean across the simulated fields
  ...
  #levels(dfa$Size) <- c(paste0("S",1:14))
  ...
  # Fix:
  displace_dat$`<NA>` <- NULL #Remove empty column
  displace_dat$SG0 <- displace_dat$SG2
  displace_dat$SG1 <- displace_dat$SG2
